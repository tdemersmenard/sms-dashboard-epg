...
export async function sendDailyReport(franchiseId: string): Promise<string[]> {
  ...
  if (!franchise?.owner_phone) {
    return ["Pas de numéro de notification configuré"];
  }
  ...
  let report = `RAPPORT JOURNALIER — ${franchise.name} — ${today}\n\n`;
  ...
  for (const contactId of uniqueContacts.slice(0, 15)) {
    ...
    if (!contact || contact.phone === franchise.owner_phone) continue;
  ...
  // 4. Envoyer le rapport au propriétaire par SMS
  ...
  if (ownerContactId) {
    ...
    await fetch(`${BASE_URL}/api/sms/send`, {
      ...
      body: JSON.stringify({ contactId: ownerContactId, body: reportToSend }),
    });
    logs.push(`[${franchise.name}] Rapport envoyé (${report.length} chars, ${conversationSummaries.length} résumés)`);
  } else {
    logs.push(`[${franchise.name}] Contact propriétaire introuvable — rapport non envoyé`);
  }
